...
pthread_cond_t buf1_ready, buf2_ready;
...
void* decode_xor(void* args) {
    ...
    while (true) {
        pthread_cond_wait(&buf1_ready, &buf1_mutex);
        pthread_cond_wait(&buf2_ready, &buf2_mutex);
...
void* read_buf1(void* args){
    ...
    while(true){
        ...
        if(buf1_index - cur_decode_index > RESYNC_LEN){
            pthread_cond_signal(&buf1_ready);
        }
...
void* read_buf2(void* args){
    ...
    while(true){
        ...
        if(buf2_index - cur_decode_index > min_match_distance){
            pthread_cond_signal(&buf2_ready);
        }
...
void decode_main(int fd1, int fd2){
    ...
    pthread_cond_init(&buf1_ready, NULL);
    pthread_cond_init(&buf2_ready, NULL);
